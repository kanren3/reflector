...
)
{
    ...
    if (0 != Length) {
        Pointer = RinNonPagedAlloc(Length);
    }
...
)
{
    ...
    if (ZYDIS_REGISTER_NONE != IdleRegister) {
        ...
        NextPointer = BuildInstructionFromRequest(
            Analyzer->Encoder.Address,
            ARRAYSIZE(Request),
            Request);

        if (NULL != NextPointer) {
            Analyzer->Encoder.Length = (ZyanUSize)(NextPointer - Pointer);
            return NextPointer;
        }
    }
